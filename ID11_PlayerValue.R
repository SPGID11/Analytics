---
  title: "ID11 Player Valuation Model"
...
standard <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "standard", team_or_player = "player")

shooting <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "shooting", team_or_player = "player")

passing <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "passing", team_or_player = "player")

passingtypes <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "passing_types", team_or_player = "player")

gca <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "gca", team_or_player = "player")
```

```{r tables2}
defense <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "defense", team_or_player = "player")
possession <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "possession", team_or_player = "player")
```


```{r, tables3}
misc <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "misc", team_or_player = "player")
playingtime <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "playing_time", team_or_player = "player")
keepers <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "keepers", team_or_player = "player")
keepers_adv <- fb_big5_advanced_season_stats(season_end_year = c(2019:2021), stat_type = "keepers_adv", team_or_player = "player")
```
...
```{r marketvalues20}
market_values20 <- get_player_market_values(country_name = c("England", "Spain", "France", "Italy", "Germany"), start_year = 2020)
```

```{r bind}
market_values <- rbind(market_values18, market_values19, market_values20)
```
...
Are these players we can remove? Non-relevant players? Most players we dont even have a valuation.  We will not include these players.  
...
Lets separate each player by position.
```{r}
...
# Removing unnecessary variables
central_midfield1 <- central_midfield %>% 
  select(-region, -PlayerYearComp_id, -Url, -Season_End_Year, -Squad, 
         -Player, -Nation, -Pos, -Age, -Born, -player_dob, -player_name, 
         -player_num, -squad, -country, -comp_name, -player_nationality, 
         -current_club, -player_height_mtrs, -date_joined, -joined_from, 
...
# Random Forest
RF_central_midfield <- randomForest(player_market_value_euro ~ .,
...
# Plot
print(RF_central_midfield)
plot(RF_central_midfield)
```
...
for(mtry in 1:9){
  RFfit_attacking_midfield <- randomForest(player_market_value_euro ~ 
                                           ...
                                           na.action = na.roughfix,
                                           ntree = 1000)
  oob_err[mtry] <- RFfit_attacking_midfield$mse[1000]
...
for(mtry in 1:9){
  RFfit_central_midfield <- randomForest(player_market_value_euro ~ 
                                         ...
                                         na.action = na.roughfix,
                                         ntree = 1000)
  oob_err[mtry] <- RFfit_central_midfield$mse[1000]
...
for(mtry in 1:9){
  RFfit_centre_back <- randomForest(player_market_value_euro ~ 
                                    ...
                                    na.action = na.roughfix,
                                    ntree = 1000)
  oob_err[mtry] <- RFfit_centre_back$mse[1000]
...
for(mtry in 1:9){
  RFfit_centreForward_secondStriker  <- randomForest(player_market_value_euro ~ 
                                                     ...
                                                     na.action = na.roughfix,
                                                     ntree = 1000)
  oob_err[mtry] <- RFfit_centreForward_secondStriker$mse[1000]
...
for(mtry in 1:9){
  RFfit_right_left_back <- randomForest(player_market_value_euro ~ 
                                        ...
                                        na.action = na.roughfix,
                                        ntree = 1000)
  oob_err[mtry] <- RFfit_right_left_back$mse[1000]
...
for(mtry in 1:9){
  RFfit_right_left_midfielder <- randomForest(player_market_value_euro ~ 
                                              ...
                                              na.action = na.roughfix,
                                              ntree = 1000)
  oob_err[mtry] <- RFfit_right_left_midfielder$mse[1000]
...
for(mtry in 1:9){
  RFfit_winger <- randomForest(player_market_value_euro ~ 
                               ...
                               na.action = na.roughfix,
                               ntree = 1000)
  oob_err[mtry] <- RFfit_winger$mse[1000]
...
varImpPlot(RFfit_right_left_back, n.var = 15)
```
### Right/Left Midfield
...
`player_foot`: The players dominant foot
...
`Gls`: Goals
  ...
  ```{r attackingmidfield-lin-reg}
attackingMidfield_linReg <- lm(player_market_value_euro ~ GCA90_GCA + Ast_Per + xGplus_per_minus,
                               data = attacking_midfield1)

summary(attackingMidfield_linReg)
```

### Center Midfield

```{r central-midfield-lin-reg}
centralMidfield_linReg <- lm(player_market_value_euro ~ SoT_per_90 + PPA + xGplus_per_minus,
                             data = central_midfield1)

summary(centralMidfield_linReg) 
```

### Center Back

```{r centre-back-lin-reg}
centreBack_linReg <- lm(player_market_value_euro ~ Int + Prog_Carries + Comp + xGplus_per_minus,
                        data = centre_back1)

summary(centreBack_linReg) 
```

### Centre Forward and Second Striker
```{r centre-forward-lin-reg}
centreForward_linReg <- lm(player_market_value_euro ~ xA + Att_3rd_Touches + PPA + xGplus_per_minus,
                           data = centreForward_secondStriker1)

summary(centreForward_linReg) 
```

### Right/Left Back

```{r rightleft-back-lin-reg}
rightLeftBack_linReg <- lm(player_market_value_euro ~ Comp + Mid_3rd + Low_Height,
                           data = right_left_back1)

summary(rightLeftBack_linReg) 
```

### Right/Left Midfield

```{r rightleft-back-lin-reg}
rightLeftMidfielder_linReg <- lm(player_market_value_euro ~ SoT_percent + player_age + Tkl + Comp,
                                 data = right_left_midfielder1)

summary(rightLeftMidfielder_linReg) 
```

### Right/Left Winger

```{r rightleft-back-lin-reg}
rightLeftWinger_linReg <- lm(player_market_value_euro ~ PPM + Int_Outcomes + xGplus_per_minus,
                             data = winger1)

summary(rightLeftWinger_linReg) 
```

# Appendix A: Important Variables
### Attacking Midfield
